    ...
    def __init__(self, llm=None, confidence_threshold=0.7, use_llm_validation=True, 
        ...
        self._clinical_domains = {
            ...
            "concomitant_medications": [
                r"concomitant\s+medication", r"concomitant\s+drug", r"concurrent\s+medication",
                r"concomitant\s+therapy", r"concomitant\s+treatment", r"co[\-\s]*medication"
            ],

            # Prior Medications (NEW)
            "prior_medications": [
                r"prior\s+medication", r"previous\s+medication", r"prior\s+therapy",
                r"previous\s+therapy", r"prior\s+treatment", r"medication\s+history"
            ],
            ...
            "physical_examination": [
                r"physical\s+exam", r"physical\s+examination", r"clinical\s+exam",
                r"body\s+system", r"physical\s+findings", r"examination\s+findings"
            ],
            ...
            # Immunogenicity (NEW)
            "immunogenicity": [
                r"immunogenicity", r"immunogenic", r"anti[\-\s]*drug\s+antibod", r"\bada\b",
                r"neutralizing\s+antibod", r"immunological\s+response", r"antibody\s+response"
            ],

            # Biomarkers (NEW)
            "biomarkers": [
                r"biomarker", r"tumor\s+marker", r"genetic\s+marker", r"molecular\s+marker",
                r"predictive\s+marker", r"prognostic\s+marker", r"pharmacodynamic\s+marker"
            ],

            # Quality of Life (NEW)
            "quality_of_life": [
                r"quality\s+of\s+life", r"\bqol\b", r"patient[\-\s]*reported\s+outcome",
                r"\bpro\b", r"health\s+status", r"functional\s+assessment"
            ],
            
            # Disposition - FIXED: Better patterns
            "disposition": [
                r"disposition", r"enrollment", r"randomization", r"completion", r"screening", 
            ...
            "inclusion_exclusion": [
                r"inclusion\s+criteria", r"exclusion\s+criteria", r"eligibility\s+criteria",
                r"entry\s+criteria", r"selection\s+criteria"
            ],

            # Protocol Deviations (NEW)
            "protocol_deviations": [
                r"protocol\s+deviation", r"protocol\s+violation", r"major\s+deviation",
                r"minor\s+deviation", r"important\s+protocol\s+deviation"
            ],

            # Pregnancy (NEW)
            "pregnancy": [
                r"pregnancy", r"pregnant", r"pregnancy\s+test", r"pregnancy\s+outcome",
                r"maternal", r"fetal", r"prenatal"
            ],

            # Mortality (NE
            "mortality": [
                r"mortality", r"death", r"fatal", r"died", r"deceased",
                r"survival", r"time\s+to\s+death"
            ],

            # Hospitalizations
            "hospitalizations": [
                r"hospitalization", r"hospital\s+admission", r"inpatient",
                r"emergency\s+room", r"\ber\s+visit", r"hospital\s+stay"
            ],

            # Dose Modifications
            "dose_modifications": [
                r"dose\s+modification", r"dose\s+adjustment", r"dose\s+reduction",
                r"dose\s+interruption", r"dose\s+escalation", r"dose\s+delay"
            ],

            # Tumor Response - Oncology
            "tumor_response": [
                r"tumor\s+response", r"recist", r"complete\s+response", r"partial\s+response",
                r"stable\s+disease", r"progressive\s+disease", r"objective\s+response",
                r"tumor\s+assessment", r"disease\s+progression"
            ],

            # Neurological Assessments
            "neurological": [
                r"neurological", r"cognitive", r"mental\s+status", r"neurologic\s+exam",
                r"mmse", r"moca", r"neuropsychological"
            ],

            # Ophthalmology
            "ophthalmology": [
                r"ophthalmology", r"ophthalmic", r"eye\s+exam", r"visual\s+acuity",
                r"retinal", r"ocular", r"vision\s+test"
            ],
            
            # Sleep Questionnaires
            "sleep_questionnaires": [
                r"epworth\s+sleepiness\s+scale", r"\bess\b", r"sleep\s+questionnaire",
                r"sleep\s+diary", r"sleep\s+quality", r"daytime\s+sleepiness",
                r"insomnia\s+severity", r"pittsburgh\s+sleep\s+quality", r"\bpsqi\b"
            ],

            # Polysomnography
            "polysomnography": [
                r"polysomnography", r"\bpsg\b", r"sleep\s+study", r"overnight\s+sleep\s+study",
                r"apnea[\-\s]*hypopnea\s+index", r"\bahi\b", r"respiratory\s+disturbance\s+index",
                r"\brdi\b", r"oxygen\s+desaturation", r"periodic\s+limb\s+movement"
            ],

            # Cohort Information - Generic patterns for title detection
            "cohort_info": [
                r"\w+\s+cohort", r"cohort:\s*\w+", r"\(\s*\w+\s+cohort\s*\)"
            ],

            # Page-By Classification - Generic patterns for BYVAL parameter/site/category headers
            "page_by": [
                r"^parameter:\s*.+", r"^site:\s*.+", r"^visit:\s*.+", r"^treatment:\s*.+",
                r"^category:\s*.+", r"^group:\s*.+"
            ],

            # ===== NEW ONCOLOGY CATEGORIES =====
            
            # Solid Tumors - General
            "solid_tumors": [
                r"solid\s+tumor", r"solid\s+malignancy", r"solid\s+cancer",
                r"tumor\s+size", r"tumor\s+volume", r"tumor\s+burden",
                r"target\s+lesion", r"non[\-\s]*target\s+lesion", r"new\s+lesion",
                r"tumor\s+measurement", r"lesion\s+assessment", r"tumor\s+diameter"
            ],

            # Gliomas and CNS Tumors
            "gliomas_cns": [
                r"\bglioma", r"glioblastoma", r"\bgbm\b", r"astrocytoma",
                r"oligodendroglioma", r"ependymoma", r"medulloblastoma",
                r"brain\s+tumor", r"brain\s+metastasis", r"brain\s+metastases",
                r"cns\s+tumor", r"central\s+nervous\s+system\s+tumor",
                r"rano", r"neuro[\-\s]*oncology", r"karnofsky", r"kps\b",
                r"neurological\s+function", r"corticosteroid\s+use"
            ],

            # Small Cell Lung Cancer
            "small_cell_lung_cancer": [
                r"small\s+cell\s+lung\s+cancer", r"\bsclc\b", r"small[\-\s]*cell\s+carcinoma",
                r"limited[\-\s]*stage\s+sclc", r"extensive[\-\s]*stage\s+sclc",
                r"ls[\-\s]*sclc", r"es[\-\s]*sclc", r"neuroendocrine\s+carcinoma"
            ],

            # Non-Small Cell Lung Cancer
            "non_small_cell_lung_cancer": [
                r"non[\-\s]*small\s+cell\s+lung\s+cancer", r"\bnsclc\b",
                r"adenocarcinoma\s+of\s+lung", r"squamous\s+cell\s+lung",
                r"lung\s+adenocarcinoma", r"egfr", r"alk\s+positive", r"ros1",
                r"pd[\-\s]*l1", r"kras", r"stage\s+iii\s+nsclc", r"stage\s+iv\s+nsclc"
            ],

            # Breast Cancer
            "breast_cancer": [
                r"breast\s+cancer", r"breast\s+carcinoma", r"mammary\s+carcinoma",
                r"her2\s+positive", r"her2[\-\s]*negative", r"triple[\-\s]*negative",
                r"tnbc", r"hormone\s+receptor\s+positive", r"er\s+positive", r"pr\s+positive",
                r"metastatic\s+breast\s+cancer", r"\bmbc\b", r"locally\s+advanced\s+breast"
            ],

            # Colorectal Cancer
            "colorectal_cancer": [
                r"colorectal\s+cancer", r"\bcrc\b", r"colon\s+cancer", r"rectal\s+cancer",
                r"metastatic\s+colorectal", r"\bmcrc\b", r"microsatellite\s+instability",
                r"\bmsi\b", r"mismatch\s+repair", r"\bmmr\b", r"kras\s+mutation",
                r"braf\s+mutation", r"left[\-\s]*sided\s+colon", r"right[\-\s]*sided\s+colon"
            ],

            # Prostate Cancer
            "prostate_cancer": [
                r"prostate\s+cancer", r"prostate\s+carcinoma", r"prostatic\s+neoplasm",
                r"castration[\-\s]*resistant", r"\bcrpc\b", r"metastatic\s+prostate",
                r"\bmcrpc\b", r"psa\s+level", r"prostate[\-\s]*specific\s+antigen",
                r"androgen\s+deprivation", r"gleason\s+score"
            ],

            # Melanoma
            "melanoma": [
                r"melanoma", r"malignant\s+melanoma", r"cutaneous\s+melanoma",
                r"metastatic\s+melanoma", r"braf\s+mutant", r"braf\s+v600",
                r"nras\s+mutation", r"mucosal\s+melanoma", r"uveal\s+melanoma",
                r"acral\s+melanoma", r"stage\s+iii\s+melanoma", r"stage\s+iv\s+melanoma"
            ],

            # Renal Cell Carcinoma
            "renal_cell_carcinoma": [
                r"renal\s+cell\s+carcinoma", r"\brcc\b", r"kidney\s+cancer",
                r"clear\s+cell\s+rcc", r"papillary\s+rcc", r"chromophobe\s+rcc",
                r"metastatic\s+rcc", r"\bmrcc\b", r"nephrectomy"
            ],

            # Hepatocellular Carcinoma
            "hepatocellular_carcinoma": [
                r"hepatocellular\s+carcinoma", r"\bhcc\b", r"liver\s+cancer",
                r"hepatic\s+carcinoma", r"child[\-\s]*pugh", r"barcelona\s+clinic",
                r"\bbclc\b", r"alpha[\-\s]*fetoprotein", r"\bafp\b", r"cirrhosis"
            ],

            # Pancreatic Cancer
            "pancreatic_cancer": [
                r"pancreatic\s+cancer", r"pancreatic\s+carcinoma", r"pancreatic\s+adenocarcinoma",
                r"metastatic\s+pancreatic", r"locally\s+advanced\s+pancreatic",
                r"ca\s+19[\-\s]*9", r"whipple\s+procedure", r"pancreaticoduodenectomy"
            ],

            # Ovarian Cancer
            "ovarian_cancer": [
                r"ovarian\s+cancer", r"ovarian\s+carcinoma", r"epithelial\s+ovarian",
                r"fallopian\s+tube\s+cancer", r"primary\s+peritoneal",
                r"platinum[\-\s]*sensitive", r"platinum[\-\s]*resistant",
                r"ca[\-\s]*125", r"brca\s+mutation", r"parp\s+inhibitor"
            ],

            # Hematologic Malignancies
            "hematologic_malignancies": [
                r"hematologic\s+malignancy", r"hematological\s+malignancy",
                r"blood\s+cancer", r"leukemia", r"lymphoma", r"myeloma",
                r"acute\s+myeloid\s+leukemia", r"\baml\b", r"acute\s+lymphoblastic\s+leukemia",
                r"\ball\b", r"chronic\s+lymphocytic\s+leukemia", r"\bcll\b",
                r"chronic\s+myeloid\s+leukemia", r"\bcml\b", r"hodgkin\s+lymphoma",
                r"non[\-\s]*hodgkin\s+lymphoma", r"\bnhl\b", r"multiple\s+myeloma",
                r"myelodysplastic\s+syndrome", r"\bmds\b"
            ],

            # Immunotherapy Response
            "immunotherapy_response": [
                r"immunotherapy", r"immune\s+checkpoint\s+inhibitor",
                r"pd[\-\s]*1\s+inhibitor", r"pd[\-\s]*l1\s+inhibitor",
                r"ctla[\-\s]*4\s+inhibitor", r"immune[\-\s]*related\s+adverse",
                r"\birae\b", r"immune[\-\s]*related\s+response", r"irrecist",
                r"pseudoprogression", r"durable\s+response"
            ],

            # Radiation Oncology
            "radiation_oncology": [
                r"radiation\s+therapy", r"radiotherapy", r"radiation\s+oncology",
                r"external\s+beam\s+radiation", r"\bebrt\b", r"stereotactic\s+body",
                r"\bsbrt\b", r"stereotactic\s+radiosurgery", r"\bsrs\b",
                r"intensity[\-\s]*modulated", r"\bimrt\b", r"radiation\s+dose",
                r"gray\s+\(gy\)", r"\bgy\b", r"fractionation"
            ],

            # Surgical Oncology
            "surgical_oncology": [
                r"surgical\s+resection", r"tumor\s+resection", r"surgical\s+margin",
                r"r0\s+resection", r"r1\s+resection", r"r2\s+resection",
                r"complete\s+resection", r"partial\s+resection", r"debulking",
                r"cytoreductive\s+surgery", r"lymph\s+node\s+dissection",
                r"sentinel\s+node", r"pathological\s+complete\s+response", r"\bpcr\b"
            ],

            # Performance Status
            "performance_status": [
                r"performance\s+status", r"ecog\s+performance", r"\becog\b",
                r"karnofsky\s+performance", r"\bkps\b", r"functional\s+status",
                r"ps\s+0", r"ps\s+1", r"ps\s+2", r"ambulatory\s+status"
            ],

            # Disease Staging
            "disease_staging": [
                r"disease\s+stage", r"cancer\s+stage", r"tumor\s+stage",
                r"tnm\s+staging", r"stage\s+i\b", r"stage\s+ii\b", r"stage\s+iii\b",
                r"stage\s+iv\b", r"early[\-\s]*stage", r"advanced[\-\s]*stage",
                r"metastatic\s+disease", r"locally\s+advanced", r"regional\s+disease"
            ],

            # Progression-Free Survival
            "progression_free_survival": [
                r"progression[\-\s]*free\s+survival", r"\bpfs\b",
                r"time\s+to\s+progression", r"\bttp\b", r"disease[\-\s]*free\s+survival",
                r"\bdfs\b", r"event[\-\s]*free\s+survival", r"\befs\b",
                r"relapse[\-\s]*free\s+survival", r"\brfs\b"
            ],

            # Overall Survival
            "overall_survival": [
                r"overall\s+survival", r"\bos\b", r"survival\s+rate",
                r"median\s+survival", r"survival\s+analysis", r"kaplan[\-\s]*meier",
                r"hazard\s+ratio", r"survival\s+probability"
            ]
        }
...
"""
    ...
    def __call__(self, nodes: List[BaseNode], **kwargs) -> List[BaseNode]:
        ...
        # CRITICAL: Store original document_ids BEFORE any processing
        original_document_ids = {}
        for i, node in enumerate(nodes):
            if hasattr(node, 'metadata') and node.metadata:
                original_document_ids[i] = node.metadata.get('document_id')
        
        print(f"   Preserved document_ids for {len(original_document_ids)} nodes")
        ...
        for i, node in enumerate(nodes):
            if isinstance(node, str):
                ...
                # Restore document_id if it existed
                if i in original_document_ids:
                    text_node.metadata['document_id'] = original_document_ids[i]
        ...
        for i, (node, metadata) in enumerate(zip(validated_nodes, metadata_list)):
            # Preserve the original document_id
            original_doc_id = original_document_ids.get(i)
            
            # Update node metadata with extracted TLF metadata
            node.metadata.update(metadata)
        
            # Restore the original document_id if it was overwritten
            if original_doc_id is not None:
                node.metadata['document_id'] = original_doc_id
                
                # VERIFICATION: Double-check it was set correctly
                if node.metadata.get('document_id') != original_doc_id:
                    print(f"❌ CRITICAL: Failed to preserve document_id for node {i}")
                    print(f"   Expected: '{original_doc_id}'")
                    print(f"   Got: '{node.metadata.get('document_id')}'")
                    # Force set it again
                    node.metadata['document_id'] = original_doc_id
            
            # Debug: Log what we're setting (but check document_id preservation)
            if metadata.get('title') and i < 3:  # Only log first 3
                print(f"  Node {i}: Setting title to '{metadata['title']}', document_id preserved: '{node.metadata.get('document_id')}'")
        
        # FINAL VERIFICATION: Ensure no document_ids were lost
        nodes_with_lost_ids = []
        for i, node in enumerate(validated_nodes):
            expected_doc_id = original_document_ids.get(i)
            actual_doc_id = node.metadata.get('document_id')
            
            if expected_doc_id is not None and actual_doc_id != expected_doc_id:
                nodes_with_lost_ids.append(i)
                print(f"❌ Node {i} lost document_id: expected '{expected_doc_id}', got '{actual_doc_id}'")
                # Fix it immediately
                node.metadata['document_id'] = expected_doc_id
        
        if nodes_with_lost_ids:
            print(f"⚠️  Fixed {len(nodes_with_lost_ids)} nodes that lost their document_id")
        else:
            print(f"✅ All nodes preserved their document_id successfully")
